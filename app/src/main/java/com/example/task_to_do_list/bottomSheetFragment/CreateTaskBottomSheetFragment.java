package com.example.task_to_do_list.bottomSheetFragment;
...
import com.example.task_to_do_list.R;
import com.example.task_to_do_list.activity.MainActivity;
import com.example.task_to_do_list.broadcastReceiver.AlarmBroadcastReceiver;
import com.example.task_to_do_list.database.DatabaseClient;
import com.example.task_to_do_list.model.Task;
...
public class CreateTaskBottomSheetFragment extends BottomSheetDialogFragment {
    ...
    public boolean validateFields() {
        ...
        else if (taskEvent.getSelectedItemPosition() == 0) {
            Toast.makeText(activity, "Please select an event", Toast.LENGTH_SHORT).show();
            return false;
        }
    ...
    private void createTask() {
        class saveTaskInBackend extends AsyncTask<Void, Void, Void> {
            ...
            protected Void doInBackground(Void... voids) {
                ...
                createTask.setEvent(taskEvent.getSelectedItem().toString());
                ...
                if (!isEdit)
                    ...
                    DatabaseClient.getInstance(getActivity()).getAppDatabase()
                            ...
                            .updateAnExistingRow(taskId, addTaskTitle.getText().toString(),
                                    ...
                                    taskEvent.getSelectedItem().toString())
                            ;
    ...
    private void setDataInUI() {
        ...
        String[] eventOptions = getResources().getStringArray(R.array.task_event_options);


        int index = -1;
        for (int i = 0; i < eventOptions.length; i++) {
            if (eventOptions[i].equalsIgnoreCase(task.getEvent())) {
                index = i;
                break;
            }
        }
